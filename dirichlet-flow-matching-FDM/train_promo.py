...
train_ds = PromoterDataset(n_tsses=100000, rand_offset=0, split='train')
val_ds = PromoterDataset(n_tsses=100000, rand_offset=0, split='valid' if not args.validate_on_test else 'test') 
...
val_loader = torch.utils.data.DataLoader(val_ds, batch_size=2*args.batch_size, shuffle=False, num_workers=args.num_workers)
