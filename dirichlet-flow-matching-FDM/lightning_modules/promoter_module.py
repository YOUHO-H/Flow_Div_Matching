...
class PromoterModule(GeneralModule):
    ...
    def validation_step(self, batch, batch_idx):
        self.stage = 'val'
        if self.trainer.current_epoch < self.args.start_val_epoch:
            # print('Skipping validation before start_val_epoch: {}'.format(self.args.start_val_epoch))
            self.lg('sp-mse', [99999])
        else:
            loss = self.general_step(batch, batch_idx)
            if self.args.validate:
                self.try_print_log()
                    # print('The best validation MSE is:', self.best_val_mse)
    ...
    def general_step(self, batch, batch_idx=None):
        ...
        if self.args.mode == 'dirichlet' or self.args.mode == 'riemannian':
            xt_init, alphas = sample_cond_prob_path(self.args, seq, self.model.alphabet_size)
            xt, prior_weights = expand_simplex(xt_init, alphas, self.args.prior_pseudocount)
            if self.stage == 'train':
                # xt_init.requires_grad = True
                # normalized Gaussian noise
                delta = 1e-4
                vec = torch.randn_like(xt_init)
                vec = vec / vec.norm(dim=(1,2), keepdim=True)
                xt_init_2 = xt_init.clone().detach() + delta * vec
                xt_2 = expand_simplex(xt_init_2, alphas, self.args.prior_pseudocount)[0]
        ...
        if self.prob_reg > 0 and self.stage == 'train':
            probs = torch.nn.functional.softmax(logits, -1)
            if self.args.mode == 'dirichlet' or self.args.mode == 'riemannian':

                logits_2 = self.model(xt_2, signal=signal, t=alphas)
                probs_2 = torch.nn.functional.softmax(logits_2, -1)
                probs_diff = (probs - probs_2) / delta
                probs_diff = torch.pow(probs_diff, 2).sum(dim=-1)
                losses = losses + self.prob_reg * probs_diff
    ...
    def on_validation_epoch_end(self):
        ...
        if self.trainer.is_global_zero:
            ...
            str_ = ''
            k_=0
            val_mse = mean_log['val_sp-mse']
            if val_mse < self.best_val_mse:
                self.best_val_mse = val_mse
            str_ += f"Best val_sp-mse: {self.best_val_mse:.6f} | "
            for key in mean_log:
                if k_ > 2 or k_ < 6:
                    if 'step' in key:
                        str_ += f"{key}: {int(mean_log[key])} | "
                    elif 'epoch' in key:
                        str_ += f"{key}: {int(mean_log[key])} | "
                    else:
                        str_ += f"{key}: {mean_log[key]:.6f} | "
                k_ += 1
            print(str_)
