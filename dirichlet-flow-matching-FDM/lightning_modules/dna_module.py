...
class DNAModule(GeneralModule):
    ...
    def general_step(self, batch, batch_idx=None):
        ...
        if self.prob_reg > 0 and self.stage == 'train':
            ...
            grad_losses_xt = torch.autograd.grad(probs, xt, 
                                                 grad_outputs=torch.ones_like(probs),
                                                 create_graph=True)[0]
            grad_losses_xt_2 = torch.pow(grad_losses_xt, 2).sum(dim=-1)
            losses = losses + self.prob_reg * grad_losses_xt_2
    ...
    def on_validation_epoch_end(self):
        ...
        if self.args.dataset_type == 'toy_sampled':
            ...
            tv = (0.5 * (torch.abs(empirical_dist - self.toy_data.probs[self.args.target_class]))).sum(-1).mean()
            ...
            mean_log.update({'val_tv': tv.cpu().item(), 'val_kl': kl.cpu().item(), 'val_rkl': rkl.cpu().item(), 'val_sanity_self_kl': sanity_self_kl.cpu().item()})
