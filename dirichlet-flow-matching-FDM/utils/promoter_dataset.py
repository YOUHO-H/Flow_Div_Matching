...
class PromoterDataset(torch.utils.data.Dataset):
    def __init__(self, seqlength=1024, split="train", n_tsses=100000, rand_offset=0):
        ...
        self.split = split
        if split == "train":
            df = self.tsses.iloc[np.isin(self.tsses['chr'].values, ['chr1', 'chr2', 'chr3', 'chr4', 'chr5', 'chr6', 'chr7'])] 
            rows_to_drop = df[df['chr'] == 'chr9'].sample(frac=0.2, random_state=42).index
            df = df.drop(rows_to_drop)
            self.tsses = df.sample(frac=0.5, random_state=42)
        elif split == "valid":
