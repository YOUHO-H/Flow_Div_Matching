...
class PolynomialConvexScheduler(ConvexScheduler):
    ...
    def __init__(self, n: Union[float, int]) -> None:
        assert isinstance(
            n, (float, int)
        ), print("`n` must be a float or int. Got {}.".format(type(n)))
        assert n > 0, print("`n` must be positive. Got n={}.".format(n))
...
class PolynomialVPScheduler(Scheduler):
    """Linear Variance Preserving Scheduler."""
