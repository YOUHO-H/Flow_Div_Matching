...
def setup_training_arguments(args: ArgsNamespace) -> Dict[str, Any]:
    ...
    training_args['lambda_FDM'] = args.FDM
    out_path = os.path.join(training_args["config"]["output_dir"], training_args["run_name"])
    training_args["config"]['training']['out_path'] = out_path
...
def training_loop(
    ...
    trainer.train(model=model, logger=logger, evaluator=evaluator, test_only=True)
