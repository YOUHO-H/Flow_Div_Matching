...
from userfm import flow_matching
from userdiffusion import ode_datasets, samplers, unet
...
def train_and_evaluate(config, workdir):
  ...
  # <redacted>: why only keep the first `timesteps` values of the trajectory? just integrate to burn-in plus timesteps?
  ...
  noise = getattr(flow_matching, config.noisetype)
  difftype = getattr(flow_matching, config.difftype)(noise)
  ...
  ## train the model
  score_fn = flow_matching.train_flow_matching(
      model,
      dataiter,
      data_std,
      config.epochs,
