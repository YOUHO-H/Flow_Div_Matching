...
class ODEDataset(object):
  ...
  def generate_trajectory_data(self,
    ...
    k = len(mesh.devices)
    with mesh:
      # <redacted>: if trajectories is smaller than original batch size and k >= 2, then more than N trajectories will be generated.
      for _ in tqdm(range(0, trajectories, bs * k)):
        ...
        # <redacted>: drop burn-in time step values
