...
def compute_nll(diffusion,
  ...
  qf = odeint(prob_flow, q0, jnp.array([diffusion.tmin, diffusion.tmax]), rtol=1e-3)[-1]
  ...
  return xf, -logpx0, -logpx0 / flat_data.shape[-1]
...
def event_scores(diffusion,
  ...
  def conditioned_scores(xt, t):
    ...
    def log_p(xt):  # log p(E|xt)
      ...
      # <redacted>: What should SigmaDC's shape be?
